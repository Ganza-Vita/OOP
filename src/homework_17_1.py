...
class Category:
    ...
    def middle_price(self):
        ...
        try:
            if len(self.__products) == 0:  # Проверка на отсутствие товаров
                raise ValueError("Нет товаров в категории.")  # Генерация исключения, если товаров нет

            total_price = sum(product.price for product in self.__products)  # Сумма цен всех товаров
            return total_price / len(self.__products)  # Средняя цена
